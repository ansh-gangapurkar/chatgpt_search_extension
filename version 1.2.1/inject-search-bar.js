(function() {
    async function saveChatData(chatData) {
        try {
            ...
            if (error.message.includes('Extension context invalidated') || error.message.includes('Database not initialized')) {
    ...
    if (!searchBarContainer) {
      ...
      searchBarContainer.innerHTML = `
        <input type="text" id="chatgpt-search-input" placeholder="Search chats...">
        <div id="chatgpt-search-results"></div>
      `;
  ...
  window.onload = function() {
    ...
    async function scrapeChatTitles() {
        ...
        try {
            ...
            const updatedData = mergeAndUpdateChatData(Array.isArray(existingData) ? existingData : [], newChatData);
    ...
    function performSearch() {
        ...
        getChatData()
            ...
            .catch(error => {
                resultsContainer.innerHTML = '<div class="chatgpt-search-message">Error: Unable to perform search. Please refresh the page to try again.</div>';
